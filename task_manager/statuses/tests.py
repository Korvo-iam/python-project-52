...
class StatusCRUDTest(TestCase):
    ...
    def test_create_status_message(self):  # проверка flash успешного создания
        response = self.client.post(reverse('statuses:create'), {'name': 'Flash тест'}, follow=True)  # noqa: E501
    ...
    def test_update_status_message(self):  # проверка flash успешного обновления
        ...
        response = self.client.post(reverse('statuses:update', args=[status.id]), {'name': 'Новый'}, follow=True)  # noqa: E501
        messages = list(get_messages(response.wsgi_request))
    ...
    def test_delete_status_message(self):  # проверка flash успешного удаления
        ...
        response = self.client.post(reverse('statuses:delete', args=[status.id]), follow=True)  # noqa: E501
        messages = list(get_messages(response.wsgi_request))
    ...
    def test_create_status(self):  # проверка создания статуса
        response = self.client.post(reverse('statuses:create'), {'name': 'Тестовый статус'})  # noqa: E501
    ...
    def test_update_status(self):  # проверка обновления статуса
        ...
        response = self.client.post(reverse('statuses:update', args=[status.id]), {'name': 'Новый статус'})  # noqa: E501
        self.assertEqual(response.status_code, 302)
    ...
    def test_delete_status(self):  # проверка удаления статуса
        ...
        response = self.client.post(reverse('statuses:delete', args=[status.id]))  # noqa: E501
        self.assertEqual(response.status_code, 302)
        self.assertFalse(Status.objects.filter(name='Удаляемый статус').exists())  # noqa: E501

    def test_access_requires_login(self):  # проверка на безуспешность просмотра списка статусов неавторизованным пользователем  # noqa: E501
        self.client.logout()  # выходим из аккаунта
        response = self.client.get(reverse('statuses:list'))
        self.assertRedirects(response, '/login/?next=/statuses/')  # редирект на логин  # noqa: E501
